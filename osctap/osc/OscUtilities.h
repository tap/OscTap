...
OSCTAP_BITCAST_CONSTEXPR void FromInt32( char *p, int32_t x )   { StoreBigEndian32( p, BitCast<uint32_t>(x) ); }
constexpr void FromUInt32( char *p, uint32_t x ) { StoreBigEndian32( p, x ); }
OSCTAP_BITCAST_CONSTEXPR void FromInt64( char *p, int64_t x )   { StoreBigEndian64( p, BitCast<uint64_t>(x) ); }
constexpr void FromUInt64( char *p, uint64_t x ) { StoreBigEndian64( p, x ); }
...
OSCTAP_BITCAST_CONSTEXPR int32_t  ToInt32( const char *p )  { return BitCast<int32_t>( LoadBigEndian32( p ) ); }
constexpr uint32_t ToUInt32( const char *p ) noexcept { return LoadBigEndian32( p ); }
OSCTAP_BITCAST_CONSTEXPR int64_t  ToInt64( const char *p )  { return BitCast<int64_t>( LoadBigEndian64( p ) ); }
constexpr uint64_t ToUInt64( const char *p ) noexcept { return LoadBigEndian64( p ); }
}
