...
static std::vector<char> BuildMessage() {
    ...
    p << osctap::BeginMessage("/x") << (int32_t)1 << (float)2.5f << "hi" << true << osctap::EndMessage();
...
static std::vector<char> BuildBundle() {
    char                         tmp[256];
    osctap::OutboundPacketStream p(tmp, sizeof(tmp));
    p << osctap::BeginBundleImmediate() << osctap::BeginMessage("/a") << (int32_t)7 << osctap::EndMessage()
      << osctap::BeginMessage("/b") << "yo" << osctap::EndMessage() << osctap::EndBundle();
    return std::vector<char>(p.Data(), p.Data() + p.Size());
}
...
int main() {
    ...
    // non-multiple-of-4 total size
    {
        auto b = msg;
        b.push_back('x');
        Differential("size not mult of 4", b);
    }
    ...
    if (failures == 0)
        std::printf("OscValidateTest: OK (gate agrees with throwing path)\n");
