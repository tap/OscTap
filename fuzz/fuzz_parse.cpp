...
static void WalkMessage( const ReceivedMessage& m )
{
    ...
    for( ReceivedMessage::const_iterator it = m.ArgumentsBegin();
            it != m.ArgumentsEnd(); ++it ){
        const ReceivedMessageArgument& a = *it;
        switch( a.TypeTag() ){
            case INT32_TYPE_TAG:        (void)a.AsInt32Unchecked();       break;
            case FLOAT_TYPE_TAG:        (void)a.AsFloatUnchecked();       break;
            case CHAR_TYPE_TAG:         (void)a.AsCharUnchecked();        break;
            case RGBA_COLOR_TYPE_TAG:   (void)a.AsRgbaColorUnchecked();   break;
            case MIDI_MESSAGE_TYPE_TAG: (void)a.AsMidiMessageUnchecked(); break;
            case INT64_TYPE_TAG:        (void)a.AsInt64Unchecked();       break;
            case TIME_TAG_TYPE_TAG:     (void)a.AsTimeTagUnchecked();     break;
            case DOUBLE_TYPE_TAG:       (void)a.AsDoubleUnchecked();      break;
            case STRING_TYPE_TAG:       (void)a.AsStringUnchecked();      break;
            case SYMBOL_TYPE_TAG:       (void)a.AsSymbolUnchecked();      break;
            case BLOB_TYPE_TAG: {
                const void* data;
            ...
            default: break; // T/F/N/I and array markers carry no argument data
        }
    }
}
...
{
    (void)b.TimeTag();
    for( ReceivedBundle::const_iterator it = b.ElementsBegin();
            it != b.ElementsEnd(); ++it ){
        if( it->IsBundle() )
