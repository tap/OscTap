...
extern "C" JNIEXPORT jbyteArray JNICALL Java_org_osctap_demo_OscTap_buildMessage(JNIEnv* env, jclass, jstring jaddress,
                                                                                 jobjectArray args) {
    ...
    try {
        ...
        env->SetByteArrayRegion(result, 0, (jsize)p.Size(), reinterpret_cast<const jbyte*>(p.Data()));
    }
...
extern "C" JNIEXPORT jstring JNICALL Java_org_osctap_demo_OscTap_describe(JNIEnv* env, jclass, jbyteArray packet) {
    ...
    try {
        ...
        osctap::ReceivedMessage m(osctap::ReceivedPacket(reinterpret_cast<const char*>(bytes), (std::size_t)n));

        out = m.AddressPattern();
        for (auto a = m.ArgumentsBegin(); a != m.ArgumentsEnd(); ++a) {
            out += ' ';
            if (a->IsInt32())
                out += std::to_string(a->AsInt32Unchecked());
            else if (a->IsFloat())
                out += std::to_string(a->AsFloatUnchecked());
            else if (a->IsString())
                out += std::string("\"") + a->AsStringUnchecked() + "\"";
            else if (a->IsBool())
                out += a->AsBoolUnchecked() ? "true" : "false";
            else
                out += '?';
        }
    }
