...
private:
    static void CopyArg( osctap::OutboundPacketStream& p,
                         osctap::ReceivedMessage::const_iterator a )
    {
        if( a->IsInt32() )       p << a->AsInt32Unchecked();
        else if( a->IsFloat() )  p << a->AsFloatUnchecked();
        else if( a->IsString() ) p << a->AsStringUnchecked();
        else if( a->IsBool() )   p << a->AsBoolUnchecked();
    }

    void Forward( const osctap::IpEndpointName& to, const osctap::OutboundPacketStream& p,
                  const char *label )
    {
        try {
...
int main( int argc, char *argv[] )
{
    ...
    osctap::IpEndpointName pico    = (argc > 2) ? ParseEndpoint( argv[2], 9000 )
                                                : osctap::IpEndpointName( "192.168.1.50", 9000 );
    osctap::IpEndpointName android = (argc > 3) ? ParseEndpoint( argv[3], 9001 )
                                                : osctap::IpEndpointName( "192.168.1.20", 9001 );

    HubListener listener( pico, android );
    osctap::UdpListeningReceiveSocket socket(
        osctap::IpEndpointName( osctap::IpEndpointName::ANY_ADDRESS, listenPort ), &listener );
    gSocket = &socket;
