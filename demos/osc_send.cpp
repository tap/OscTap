...
{
    if( std::strcmp( tok, "T" ) == 0 ) { p << true;  return; }
    if( std::strcmp( tok, "F" ) == 0 ) { p << false; return; }

    if( std::strncmp( tok, "i:", 2 ) == 0 ) { p << (int32_t)std::atoi( tok + 2 ); return; }
    if( std::strncmp( tok, "f:", 2 ) == 0 ) { p << (float)std::atof( tok + 2 ); return; }
    if( std::strncmp( tok, "s:", 2 ) == 0 ) { const char *s = tok + 2; p << s; return; }

    // Auto: int, else float, else string.
    int32_t i; float f;
    if( ParseInt( tok, i ) )        p << i;
    else if( ParseFloat( tok, f ) ) p << f;
    else                            p << tok;
}
...
int main( int argc, char *argv[] )
{
    ...
    try {
        osctap::UdpTransmitSocket( osctap::IpEndpointName( host, port ) )
            .Send( p.Data(), p.Size() );
    ...
    std::cout << "sent " << p.Size() << " bytes to " << host << ':' << port
              << "  " << address << '\n';
    return 0;
}
